...
public class PerfData {
    ...
    public void dumpStatsAndMerge() {
        ...
        if (fileCount > 1) {
            LOG.log(Level.INFO, "File count: {0}", fileCount);
        }
        useAvg = false;
        dumpStats();
        if (fileCount > 1) {
    ...
    void dumpStats() {
        LOG.info("Phase times:");
        for (int i = 0; i <= Phases.ALL; i++) {
            Long d = parserPhaseTime.get(i);
            if (d == null) {
                continue;
            }
            LOG.log(Level.INFO, "\t" + phaseToString(i) + ":\t" + avg(d));
        }
        ...
        for (int i = Phases.INITIALIZATION; i <= Phases.ALL; i++) {
            ...
            LOG.log(Level.INFO, "Visitors from {0}", phaseToString(i));
            List<String> visitors = new ArrayList<>(vc.keySet());
            Collections.sort(visitors);
            for (String c : visitors) {
                LOG.log(Level.INFO, "\t{0}:\t{1}", new Object[] { c, avg(vc.get(c)) });
            }
        }
        LOG.log(Level.INFO, "Performance counters:");
        List<String> keys = new ArrayList<>(perfCounters.keySet());
        Collections.sort(keys);
        for (String s : keys) {
            LOG.log(Level.INFO, "\t{0}:\t{1}", new Object[] { s, avg(perfCounters.get(s)) });
        }
    }
