...
function activateWithJDK(specifiedJDK: string | null, context: ExtensionContext, log : vscode.OutputChannel, notifyKill: boolean): void {
    ...
    if (activationPending) {
        ...
        log.appendLine("Server activation requested repeatedly, ignoring...");
        return;
    }
    activationPending = true;
    // chain the restart after termination of the former process.
    if (a != null) {
        log.appendLine("Server activation initiated while in maintenance mode, scheduling after maintenance");
        a.then(() => killNbProcess(notifyKill, log)).
            then(() => doActivateWithJDK(specifiedJDK, context, log, notifyKill));
    } else {
        log.appendLine("Initiating server activation");
...
function killNbProcess(notifyKill : boolean, log : vscode.OutputChannel, specProcess?: ChildProcess) : Promise<void> {
    const p = nbProcess;
    log.appendLine("Request to kill LSP server.");
    if (p && (!specProcess || specProcess == p)) {
        ...
        return new Promise((resolve, reject) => {
            ...
            p.on('close', function(code: number) {
                log.appendLine("LSP server closed: " + p.pid)
                resolve();
            });
            log.appendLine("Killing LSP server " + p.pid);
            if (!p.kill()) {
                reject("Cannot kill");
            }
        });
    } else {
        log.append("Cannot kill: ");
        if (specProcess) {
            log.append("Requested kill on " + specProcess.pid + ", ");
        }
        log.appendLine("current process is " + (p ? p.pid : "None"));
...
function doActivateWithJDK(specifiedJDK: string | null, context: ExtensionContext, log : vscode.OutputChannel, notifyKill: boolean): void {
    ...
    log.appendLine("Launching LSP server");
    ...
    let ideRunning = new Promise((resolve, reject) => {
        ...
        log.appendLine("LSP server launching: " + p.pid);
        ...
        p.on('close', function(code: number) {
            ...
            if (collectedText != null) {
                ...
                log.appendLine("LSP server " + p.pid + " terminated with " + code);
                ...
                if (yes === reply) {
                    ...
                    maintenance = new Promise((resolve, reject) => {
                        ...
                        kill.then(() => {
                            ...
                            log.appendLine("Launching installation process: " + installProcess.pid);
                            ...
                            installProcess.on('close', function(code: number) {
                                log.appendLine("Installation completed: " + installProcess.pid);
