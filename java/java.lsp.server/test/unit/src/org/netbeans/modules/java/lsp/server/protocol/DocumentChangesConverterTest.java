...
public class DocumentChangesConverterTest extends NbTestCase {
    ...
    /**
     * 
     * Insert a text, and delete part of it, reaching past end
     */
    public void testInsertAndDeleteThroughEnd() throws Exception {
        insertInitialText();
        document.insertString(0, "112233", null);
        document.remove(3, 5);
        ...
        String removedT = converter.getRemovedParts().get(second);
        // deleted "233" plus 2 real from the original text, but those "233" do not count.
        assertEquals(CONTENT.substring(0, 2), removedT);
        */
        ...
        assertDeleteLength(first, 2);
        assertEquals("112", first.getNewText());
        String removedT = converter.getRemovedParts().get(first);
        // deleted "233" plus 2 real from the original text, but those "233" do not count.
        assertEquals(CONTENT.substring(0, 2), removedT);
    }
    
    /**
     * Inserts a text, and deletes starting part of it (+ some preceding text)
     */
    public void testInsertAndDeleteFromBeforeStart() throws Exception {
        insertInitialText();
        document.insertString(2, "112233", null);
        document.remove(0, 5);
                
        List<TextEdit> edits = converter.makeTextEdits();
        assertEquals(1, edits.size());
        TextEdit single = edits.get(0);
        assertEquals("233", single.getNewText());
        assertEquals(0, single.getStartOffset());
        // 2 chars deleted prior to insertion point + 3 chars inserted
        assertEquals(2, single.getEndOffset() - single.getStartOffset());
        
        String removed = converter.getRemovedParts().get(single);
        assertEquals("Lo", removed);
    }
    
    public void testInsertAndDeleteJustBefore() throws Exception {
        insertInitialText();
        ...
        assertEquals(1, edits.size());
        TextEdit single = edits.get(0);
        assertEquals("112233", single.getNewText());
        assertEquals(0, single.getStartOffset());
        // 2 chars deleted prior to insertion point + 3 chars inserted
        assertEquals(3, single.getEndOffset());
        
        String removed = converter.getRemovedParts().get(single);
        assertEquals("Lor", removed);
    }
    
    public void testInsertAndInsertJustBefore() throws Exception {
        insertInitialText();
        document.insertString(3, "445566", null);
        document.insertString(3, "112233", null);
                
        List<TextEdit> edits = converter.makeTextEdits();
        assertEquals(1, edits.size());
        TextEdit single = edits.get(0);
        assertEquals("112233445566", single.getNewText());
        assertEquals(3, single.getStartOffset());
        // 2 chars deleted prior to insertion point + 3 chars inserted
        assertEquals(3, single.getEndOffset());
    ...
    public void testInsertAndDeleteJustAfter() throws Exception {
        insertInitialText();
        document.insertString(3, "112233", null);
        document.remove(3 + 6, 3);
        ...
        assertEquals(3, single.getStartOffset());
        // 2 chars deleted prior to insertion point + 3 chars inserted
        assertEquals(6, single.getEndOffset());
        
        String removed = converter.getRemovedParts().get(single);
        assertEquals("em ", removed);
    }
    
    public void testInsertAndReplaceJustBefore() throws Exception {
        insertInitialText();
        document.insertString(3, "445566", null);
        document.remove(2, 1);
        document.insertString(2, "112233", null);
        ...
        TextEdit single = edits.get(0);
        assertEquals("112233445566", single.getNewText());
        assertEquals(2, single.getStartOffset());
        // 2 chars deleted prior to insertion point + 3 chars inserted
        assertEquals(3, single.getEndOffset());
    ...
    /**
     * Creates a combined edit (both delete and insert = replace) and inserts
     * additional text within it.
     * @throws Exception 
     */
    public void testReplaceAndInsertWithin() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        document.insertString(2, "aaa", null);
                
        List<TextEdit> edits = converter.makeTextEdits();
        assertEquals(1, edits.size());
        TextEdit edit = edits.get(0);
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("11aaa2233", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem", removed);
    }
    
    public void testReplaceAndInsertFromStart() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        ...
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("aaa112233", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem", removed);
    }
    
    public void testReplaceAndInsertFromEnd() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        document.insertString(6, "aaa", null);
        ...
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("112233aaa", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem", removed);
    }
    
    public void testReplaceAndDeleteFromStart() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        document.remove(0, 4);
        ...
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("33", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem", removed);
    }
    
    public void testReplaceAndDeleteToEnd() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        document.remove(3, 3);
        ...
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("112", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem", removed);
    }
    
    public void testReplaceAndDeleteFromEnd() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        document.remove(6, 3);
        ...
        assertEquals(2, edits.size());
        TextEdit edit = edits.get(0);
        TextEdit del = edits.get(1);
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("112233", edit.getNewText());
        
        assertEquals(5, del.getStartOffset());
        assertEquals(8, del.getEndOffset());

        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem", removed);

        String removed2 = converter.getRemovedParts().get(del);
        assertEquals(" ip", removed2);
        */

        assertEquals(1, edits.size());
        TextEdit edit = edits.get(0);
        assertEquals(0, edit.getStartOffset());
        assertEquals(8, edit.getEndOffset());
        assertEquals("112233", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem ip", removed);
    }
    
    public void testReplaceAndDeleteInside() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        document.remove(2, 3);
        ...
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("113", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem", removed);
    }
    
    public void testReplaceAndDeletePastEnd() throws Exception {
        insertInitialText();
        document.remove(0, 5);
        document.insertString(0, "112233", null);
        document.remove(4, 6);
        ...
        assertEquals(0, edit.getStartOffset());
        assertEquals(5, edit.getEndOffset());
        assertEquals("1122", edit.getNewText());
        
        String removed = converter.getRemovedParts().get(edit);
        String removed2 = converter.getRemovedParts().get(del);
        assertEquals("Lorem", removed);

        assertEquals(" ips", removed2);
        */
        assertEquals(1, edits.size());
        TextEdit edit = edits.get(0);
        assertEquals(0, edit.getStartOffset());
        assertEquals(9, edit.getEndOffset());
        assertEquals("1122", edit.getNewText());
        String removed = converter.getRemovedParts().get(edit);
        assertEquals("Lorem ips", removed);
    }
    
    public void testReplaceAndDeleteBeforeStart() throws Exception {
        insertInitialText();
        document.remove(5, 5);
        document.insertString(5, "112233", null);
        document.remove(2, 5);
        ...
        assertEquals(10, edit.getEndOffset());
        assertEquals("2233", edit.getNewText());
        
        assertEquals("rem ipsu", removed);
    }

    public void testReplaceAndDeleteBeforeAndAfter() throws Exception {
        insertInitialText();
        document.remove(5, 5);
        document.insertString(5, "112233", null);
        document.remove(2, 12);
        ...
        assertEquals(2, edit.getStartOffset());
        assertEquals(13, edit.getEndOffset());
        assertNull(edit.getNewText());
        
        assertEquals("rem ipsum d", removed);
    }
    ...
    public void testCoalesceReverseEdits() throws Exception {
        insertInitialText();
        document.insertString(3, "112233", null);
        document.insertString(3, "000", null);
