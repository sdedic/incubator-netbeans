...
//        long id = ((Requests.StepArguments) arguments).threadId;
        String step; 
        switch (command) {
            case STEPIN:
                step = "stepInto";
        ...
        /*
        JPDAThread thread = dbg.getThread(id);
        if (thread == null) {
            thread = dbg.getCurrentThread();
        }
        if (thread != null) {
            step.addPropertyChangeListener((ev) -> {
                long suspendedId = dbg.getCurrentThread().getID();
                assert id == suspendedId;
                dbg.suspend();
                context.getProtocolServer().sendEvent(new Events.StoppedEvent("step", id));
            });
            step.addStep(thread);
            thread.resume();
        }
*/
    ...
    public CompletableFuture<Response> handle_ORIG(Command command, Arguments arguments, Response response,
            IDebugAdapterContext context) {
        if (context.getDebugSession() == null) {
            return AdapterUtils.createAsyncErrorResponse(response, ErrorCode.EMPTY_DEBUG_SESSION, "Debug Session doesn't exist.");
        }
