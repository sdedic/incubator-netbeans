 ...
 * removed from the mainline processing after stabilization. The following should be exactly the same as the current live
 * document content:
 * <ul>
...
public class TwoWayMerger {
    ...
    private final ContentSnapshot liveDocument;
    ...
    public TwoWayMerger(ContentSnapshot snapshot, ContentSnapshot liveDocument, List<TextEdit> localEdits) {
    ...
    private void textEdits2Events() {
        for (TextEdit edit : updatedClient) {
            Position start = liveDocument.offset2Position(edit.getStartOffset());
            Position end = edit.getEndOffset() > edit.getStartOffset() ? liveDocument.offset2Position(edit.getEndOffset()) : start;
    ...
    /**
     * The offset to add for all positions, adjusts with each edit
     * merged in.
     */
    ...
    private void addLocal(TextEdit local) {
        if (localOffset == 0) {
            updatedLocal.add(local);
        } else {
            TextEdit t = new TextEdit(local.getStartOffset() + localOffset, local.getEndOffset() + localOffset, local.getNewText());
            updatedLocal.add(t);
        }
    ...
    /**
     * Check that the client events, when applied to the updated snapshot, will
     * match the live document state
     */
    public void checkConsistency() {
        ...
        StringBuilder sbLocal = new StringBuilder(liveDocument.getText());
