 ...
 * @author sdedic
 */
public class DocumentChangesConverterCopy implements DocumentListener {
    private final Document document;
    private boolean adjustPositions;

    public DocumentChangesConverterCopy(Document document) {
    ...
    static class CapturedTextEdit {
        final Position start;
        final Position end;
        final String newText;

        public CapturedTextEdit(Position start, Position end, String newText) {
            this.start = start;
            this.end = end;
    ...
    private List<CapturedTextEdit>  capturedEdits = new ArrayList<>();
    ...
    public synchronized boolean isEmpty() {
        return capturedEdits.isEmpty();
    }
    
    protected Position createPosition(int offset, Position.Bias bias) throws BadLocationException {
        return NbDocument.createPosition(document, offset, Position.Bias.Forward);
    }
    
    public synchronized void adjustEdits(Function<TextEdit, Integer> modifier) throws BadLocationException {
        List<TextEdit> ordered = new ArrayList<>(recordedEdits);
        ordered.sort(textEditComparator(recordedEdits));
        for (int i = 0; i < ordered.size(); i++) {
            TextEdit e = ordered.get(i);
    ...
    public void insertUpdate(DocumentEvent e) {
        ...
        try {
            ...
            Position s = createPosition(e.getOffset(), Position.Bias.Forward);
            CapturedTextEdit cte = new CapturedTextEdit(s, s, addedText);
            TextEdit edit = new TextEdit(e.getOffset(), e.getOffset(), addedText);
            synchronized (this) {
                empty = capturedEdits.isEmpty();
                recordedEdits.add(edit);
                capturedEdits.add(cte);
                //recordedEdits.add(edit);
            }
...
//        try {
            /*
            Position start = createPosition(e.getOffset(), Position.Bias.Forward);
            Position end = createPosition(e.getOffset() + e.getLength(), Position.Bias.Backward);
            CapturedTextEdit cte = new CapturedTextEdit(start, end, removedText);
            */
            synchronized (this) {
                empty = capturedEdits.isEmpty();
                //capturedEdits.add(cte);
...
//        }
    ...
    private void fixCapturedEdits() {
        /*
        for (CapturedTextEdit cte : capturedEdits) {
            int s = cte.getStart().getOffset();
            int e = cte.getEnd().getOffset();
            TextEdit te = new TextEdit(s, e, e > s ? null : cte.getChangedText());
            recordedEdits.add(te);
            if (e > s) {
                removedStrings.put(te, cte.getChangedText());
            }
        }
        */
        capturedEdits.clear();
