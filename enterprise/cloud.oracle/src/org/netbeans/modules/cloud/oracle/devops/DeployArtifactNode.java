...
public class DeployArtifactNode extends CloudNode {
    ...
    @NodeProvider.Registration(path = "Oracle/DeployArtifact")
        ...
        return project -> {
            try ( DevopsClient client = new DevopsClient(OCIManager.getDefault().getConfigProvider())) {
                ...
                return Collections.singletonList(
                        new DeployArtifactItem.DeployArtifactFolder(OCID.of(project.getKey().getValue(), "Oracle/DeployArtifactFolder"),
                                Bundle.DeployArtifacts(),
                                projects.stream()
                                        .map(p -> new DeployArtifactItem(OCID.of(p.getId(), "Oracle/DeployArtifact"), p.getDisplayName()))
    ...
    @NodeProvider.Registration(path = "Oracle/DeployArtifactFolder")
    public static NodeProvider<DeployArtifactItem.DeployArtifactFolder> createFolderNode() {
        return DeployArtifactFolderNode::new;
    }

    @ChildrenProvider.Registration(parentPath = "Oracle/DeployArtifactFolder")
