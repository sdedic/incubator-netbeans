...
public class RepositoryNode extends OCINode {
    ...
    @NodeProvider.Registration(path = "Oracle/Repository")
        ...
        return project -> {
            try ( DevopsClient client = new DevopsClient(OCIManager.getDefault().getConfigProvider())) {
                ...
                return Collections.singletonList(
                        new RepositoryFolder(OCID.of(project.getKey().getValue(), "Oracle/RepositoryFolder"),
                                Bundle.Repositories(),
                                projects.stream()
                                        .map(p -> new RepositoryItem(OCID.of(p.getId(), "Oracle/Repository"), p.getName()))
    ...
    @NodeProvider.Registration(path = "Oracle/RepositoryFolder")
    public static NodeProvider<RepositoryFolder> createFolderNode() {
        return RepositoryFolderNode::new;
    }

    @ChildrenProvider.Registration(parentPath = "Oracle/RepositoryFolder")
